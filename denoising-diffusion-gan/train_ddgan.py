...
def train_iter(data_loader, netG, netD, optimizerG, optimizerD, global_step, accum_iter, batch_size, nz, device, coeff, pos_coeff):
    ...
    for iteration, (x, y) in enumerate(data_loader):
        ...
        errD_real = errD_real / accum_iter
        ...
        if args.lazy_reg is None:
            ...
            if global_step % args.lazy_reg == 0:
                ...
                grad_real = grad_real / accum_iter
                grad_penalty = (
                        grad_real.view(grad_real.size(0), -1).norm(2, dim=1) ** 2
                ).mean()
        ...
        errD_fake = errD_fake / accum_iter
        errD_fake.backward()

        errD = errD_real + errD_fake
        # Update D
        if ((iteration + 1) % accum_iter == 0) or (iteration + 1 == len(data_loader)):
            optimizerD.step()
        ...
        if iteration % 100 == 0:
            if rank == 0:
                ...
                print(f"100 iterations took {time.time() - s}\n")
                s = time.time()
...
def train(rank, gpu, args):
    ...
    exp = args.exp
    parent_dir = "./saved_info_custom/dd_gan/{}".format(args.dataset)
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        print(f"data len: {len(data_loader)}")
        accum_iter = 20

        train_iter(data_loader, netG, netD, optimizerG, optimizerD, global_step, accum_iter, batch_size, nz, device, coeff, pos_coeff)
